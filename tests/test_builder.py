...
def test_user_tool_result_blocks_content():
    b = MessageBuilder().user_tool_result("call_1", [{"type": "text", "text": "done"}])
    block = b.build()[0]["content"][0]
...
def test_copy_is_deeply_independent():
    ...
    fork = base.copy()
    # Mutating the fork's nested block must not leak into the original.
    fork.build()  # sanity: fork has the seeded block
    fork._messages[0]["content"][0]["text"] = "changed"
    assert base.build()[0]["content"][0]["text"] == "shared"


def test_extend_does_not_share_state():
